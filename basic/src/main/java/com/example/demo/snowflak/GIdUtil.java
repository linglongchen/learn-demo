...
public class GIdUtil {
     ...
     *            数据中心ID (0~63)
     */
    public GIdUtil(long datacenterId,String ips) {
        ...
        try {
            ...
            workerId2 = Long.valueOf(ip[ip.length - 2]).longValue();
        ...
        StringBuffer str = new StringBuffer(String.valueOf(workerId));
        str.append(workerId2);

        this.workerId = Long.parseLong(str.toString());
    ...
    public synchronized long nextId() {
        ...
        StringBuffer stringBuffer = new StringBuffer(String.valueOf(datacenterId));
        stringBuffer.append(workerId);
        int i = stringBuffer.hashCode();
        int j = i & 0x0000FF00;
//        int k = j << 8;
        return Long.parseLong(String.valueOf(j)) % 32;
//        return Long.parseLong(String.valueOf(((timestamp - twepoch) << timestampLeftShift | (stringBuffer.hashCode() & 0x0000FF00) << 8) | sequence).replace("-", ""));
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1;i<255;i++) {
            String ip = "10.105."+random.nextInt(255)+"."+random.nextInt(255);
            GIdUtil gIdUtil = new GIdUtil(5,ip);
            Long id = gIdUtil.nextId();
            if (set.contains(id)) {
                count++;
            } else {
                set.add(id);
            }
            System.out.println("=====ip:"+ip+"======数值："+id+"======count:"+count);
        }
    }
